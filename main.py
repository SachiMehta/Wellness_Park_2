...
def den():
  ...
  if request.method == "POST" and "inputTodo" in request.form:
    ...
    if input.isspace() or input == None or input == "":
      rows = db.execute("SELECT * FROM 'todo' WHERE user_id = ?",
                        session["user_id"])
      print(rows)
      return render_template("den.html", open=True, rows=rows)
    db.execute(
        "INSERT INTO todo (user_id, task, checked) VALUES(:user_id, :task, :checked)",
  ...
  if request.method == "POST" and "mood" in request.form:
    ...
    setMood = ""
    rows = db.execute(
        "SELECT * FROM mood WHERE date = :date AND user_id = :user_id AND via = 'tracker' ",
        date=day,
        user_id=session['user_id'])

    try:
      db.execute(
          "UPDATE 'mood' SET 'mood' = :mood WHERE user_id = :user_id AND date = :date AND via = 'tracker'",
          mood=input,
          user_id=session["user_id"],
          date=day)
      setMood = rows[0]['mood']
      ...
      db.execute(
          "INSERT INTO 'mood' (user_id, via, date, mood) VALUES (:user_id, :via, :date, :mood)",
          user_id=session['user_id'],
          via='tracker',
          date=day,
          mood=int(input))
  ...
  if request.method == "POST" and "journalQuestion" in request.form:
    ...
    apiKey = db.execute(
        "SELECT * FROM journal WHERE date = :date AND user_id = :user_id AND author = 'api'",
        date=day,
        user_id=session['user_id'])
    ...
    if not input.isspace() and answer != "":
      db.execute(
          "INSERT INTO journal (user_id, text, date, author) VALUES(:user_id, :text, :date, 'Me')",
          user_id=session['user_id'],
          text=input,
          date=day)
      db.execute(
          "INSERT INTO journal (user_id, text, date, author) VALUES(:user_id, :text, :date, 'AI')",
          user_id=session['user_id'],
          text=answer,
          date=day)
    ...
    userText = ""
    userText_rows = db.execute(
        "SELECT * FROM journal WHERE date = :date AND user_id = :user_id AND author = 'Me' ",
        date=day,
        user_id=session['user_id'])
    ...
    if sentiment_results['neg'] == 0 and sentiment_results[
        'neu'] == 0 and sentiment_results['pos'] == 0:
  ...
  if request.method == "POST" and "apiKey" in request.form:
    ...
    rows = db.execute(
        "SELECT * FROM journal WHERE user_id = :user_id AND author = 'api' ",
        user_id=session["user_id"])
    if len(rows) > 0:
      print("updateed api key")
      db.execute(
          "UPDATE 'journal' SET 'text' = :text WHERE user_id = :user_id AND date = :date AND author = 'api' ",
          text=input,
          user_id=session["user_id"],
          date=day)
    else:
      db.execute(
          "INSERT INTO journal (user_id, text, date, author) VALUES(:user_id, :text, :date, 'api')",
          user_id=session['user_id'],
          text=input,
          date=day)
  ...
  if request.method == "POST" and "updateView" in request.form:
    ...
    if input == "open3":  #journal
      ...
      db.execute("DELETE FROM journal WHERE date <> :date",
                 date=day)  # ERROR? <> means not equal to
      #mood setting TODO
      ...
      mood_rows = db.execute(
          "SELECT * FROM mood WHERE user_id = :user_id AND date = :date AND via = 'journal' ",
          user_id=session["user_id"],
          date=day)
      if len(mood_rows) > 0:
        print("updateed mood")
        db.execute(
            "UPDATE 'mood' SET 'mood' = :mood WHERE user_id = :user_id AND date = :date AND via = :via",
            mood=sentiment,
            user_id=session["user_id"],
            date=day,
            via="journal")
    ...
    if input == "open4":  #chart
      ...
      journal_mood_rows = db.execute(
          "SELECT * FROM mood WHERE user_id = :user_id AND via = 'journal' ",
          user_id=session["user_id"])
      tracker_mood_rows = db.execute(
          "SELECT * FROM mood WHERE user_id = :user_id AND via = 'tracker' ",
          user_id=session["user_id"])
